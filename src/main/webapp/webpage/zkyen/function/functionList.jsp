...
<div region="center" style="padding: 1px;">
    <t:datagrid name="functionList" title="menu.manage"
                actionUrl="zkyenmenuController.do?functionGrid" idField="id" treegrid="true" pagination="false">
	    ...
	    <t:dgDelOpt exp="delAccess#eq#1" url="zkyenmenuController.do?del&id={id}" title="common.delete"></t:dgDelOpt>
        <t:dgToolBar title="common.add.param" langArg="common.menu" icon="icon-add" url="zkyenmenuController.do?addorupdate" funname="addFun"></t:dgToolBar>
        <t:dgToolBar title="common.edit.param" langArg="common.menu" icon="icon-edit" url="zkyenmenuController.do?addorupdate" funname="update"  height="550"></t:dgToolBar>
    </t:datagrid>
</div>
</div>
<%--   update-start--Author:duanql  Date:20130619 for：操作按钮窗口显示控制--%>
...
function  operationData(fucntionId){
	...
	$('#operationDetailpanel').panel("refresh", "zkyenmenuController.do?dataRule&functionId=" +fucntionId);
}
function operationDetail(functionId)
{
	...
	$('#operationDetailpanel').panel("refresh", "zkyenmenuController.do?operation&functionId=" +functionId);
}
