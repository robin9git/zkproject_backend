...
<script type="text/javascript">
	...
	$(function() {
		$('#cc').combotree({
			url : 'zkyenmenuController.do?setPFunction&selfId=${function.id}',
			panelHeight: 200,
			width: 157,
...
<t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="zkyenmenuController.do?saveFunction">
	<input name="id" type="hidden" value="${function.id}">
	<input name="del_access" type="hidden" value="${function.del_access}">
	<fieldset class="step">
	<div class="form">
        <label class="Validform_label"> <t:mutiLang langKey="menu.name"/>: </label>
        <input name="functionName" class="inputxt" value="${function.functionName}" datatype="*">
	...
	<%--add-start--Author:zhouzhihua  Date:20150807 for：备注--%>
	<div class="form"><label class="Validform_label"> <t:mutiLang langKey="menu.functionMark"/>: </label> <input name="functionMark" class="inputxt" value="${function.functionMark}"></div>
	<%--add-end--Author:zhouzhihua  Date:20150807 for：备注--%>
	
	<div class="form">
        <label class="Validform_label"> 菜单显示位置: </label>
        <t:dictSelect field="show_type" typeGroupCode="show_type" hasLabel="false" defaultVal="${function.show_type!='' && function.show_type!=null?function.show_type:4 }"></t:dictSelect>
        <span class="Validform_checktip"></span>
    </div>
