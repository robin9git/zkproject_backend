...
public class CkfinderTag extends TagSupport {
	...
	protected String size;// 文件大小 KB
	
	protected boolean mutipUpload;//是否允许多张图片上传 默认false
	...
	public boolean isMutipUpload() {
		return mutipUpload;
	}

	public void setMutipUpload(boolean mutipUpload) {
		this.mutipUpload = mutipUpload;
	}
...
//			sb.append(" />");
			...
			sb.append("<input type=\"hidden\" id=\"" + name + "_input\" name=\""
					+ name + "\" value=\"" + value + "\">");
			if(mutipUpload){
				sb.append("<ol id='preview'></ol>");
				sb.append("<input class=\"" + buttonClass
						+ "\" type=\"button\" value=\"" + buttonValue
						+ "\" onclick=\"browseImg('" + name + "_input','" + name
						+ "_herf','" + uploadType + "')\">");
				sb.append("<input class=\"" + buttonClass
						+ "\" type=\"button\" value=\"" + "清除"
						+ "\" onclick=\"DelAll('" + name + "_input')\"> <span style='color:red'> (注意：多张图片需一次上传完毕)</span>");
			}else{
				sb.append("<img src=\"" + value + "\" width=\"" + width
						+ "\" height=\"" + height + "\" id=\"" + name + "_herf\" ");
				if (StringUtil.isEmpty(value))
			...
			if(StringUtil.isNotEmpty(size)){
				//设置文件大小 KB
				sb.append("<input type=\"hidden\" id=\"" + size + "_input\" name=\""
						+ size + "\" value=\"" + 0 + "\">");
				
				sb.append("<input class=\"" + buttonClass
						+ "\" type=\"button\" value=\"" + buttonValue
						+ "\" onclick=\"browseWSize('" + name + "_input','" + name
						+ "_herf','" + uploadType + "','" + size + "_input')\">");
			}else{
				sb.append("<input class=\"" + buttonClass
						+ "\" type=\"button\" value=\"" + buttonValue
						+ "\" onclick=\"browse('" + name + "_input','" + name
