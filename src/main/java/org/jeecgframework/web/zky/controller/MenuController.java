...
public class MenuController extends BaseController {
	...
	public void datagrid(HttpServletRequest request,
		...
		CriteriaQuery cq = new CriteriaQuery(ZkZhMenu.class, dataGrid);
	...
	@ResponseBody
	public AjaxJson del(ZkZhMenu function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		function = systemService.getEntity(ZkZhMenu.class, function.getId());
	 ...
	 * @param parent
	 */
	private void updateSubFunction(List<ZkZhMenu> subFunction,ZkZhMenu  parent) {
		if(subFunction.size() ==0){//没有子菜单是结束
              return;
       }else{
    	   for (ZkZhMenu tsFunction : subFunction) {
				tsFunction.setFunctionLevel(Short.valueOf(parent.getFunctionLevel()
						+ 1 + ""));
				systemService.saveOrUpdate(tsFunction);
				List<ZkZhMenu> subFunction1 = systemService.findByProperty(ZkZhMenu.class, "TSFunction.id", tsFunction.getId());
	...
	@ResponseBody
	public AjaxJson saveFunction(ZkZhMenu function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		...
		if (function.getTSFunction().getId().equals("")) {
			...
			ZkZhMenu parent = systemService.getEntity(ZkZhMenu.class,
		...
		if (StringUtil.isNotEmpty(function.getId())) {
			...
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
			List<ZkZhMenu> subFunction = systemService.findByProperty(ZkZhMenu.class, "TSFunction.id", function.getId());
...
//			systemService.flushRoleFunciton(function.getId(), function);
			...
			if (function.getFunctionLevel().equals(Globals.Function_Leave_ONE)) {
				List<ZkZhMenu> functionList = systemService.findByProperty(
						ZkZhMenu.class, "functionLevel",
	...
	public ModelAndView addorupdate(ZkZhMenu function, HttpServletRequest req) {
		String functionid = req.getParameter("id");
		List<ZkZhMenu> fuinctionlist = systemService
				.getList(ZkZhMenu.class);
		...
		if (functionid != null) {
			function = systemService.getEntity(ZkZhMenu.class, functionid);
		...
		if (function.getTSFunction() != null
			...
			function.setTSFunction((ZkZhMenu) systemService.getEntity(
					ZkZhMenu.class, function.getTSFunction().getId()));
			req.setAttribute("function", function);
		}
	...
	public List<TreeGrid> functionGrid(HttpServletRequest request,
			TreeGrid treegrid) {
		CriteriaQuery cq = new CriteriaQuery(ZkZhMenu.class);
...
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
		...
		cq.add();
		List<ZkZhMenu> functionList = systemService.getListByCriteriaQuery(cq, false);
	...
	public void functionList(HttpServletRequest request,
		...
		cq.add();
		List<ZkZhMenu> functionList = systemService.getListByCriteriaQuery(
				cq, false);
		List<TreeGrid> treeGrids = new ArrayList<TreeGrid>();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
...
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
	...
	public ModelAndView searchApp(ZkZhMenu function, HttpServletRequest req) {
