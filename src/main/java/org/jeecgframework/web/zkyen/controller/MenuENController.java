...
@Scope("prototype")
@Controller
@RequestMapping("/zkyenmenuController")
public class MenuENController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(MenuENController.class);
	...
	public void datagrid(HttpServletRequest request,
		...
		CriteriaQuery cq = new CriteriaQuery(MenuEN.class, dataGrid);
	...
	@ResponseBody
	public AjaxJson del(MenuEN function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		function = systemService.getEntity(MenuEN.class, function.getId());
	 ...
	 * @param parent
	 */
	private void updateSubFunction(List<MenuEN> subFunction,MenuEN  parent) {
		if(subFunction.size() ==0){//没有子菜单是结束
              return;
       }else{
    	   for (MenuEN tsFunction : subFunction) {
				tsFunction.setFunctionLevel(Short.valueOf(parent.getFunctionLevel()
						+ 1 + ""));
				systemService.saveOrUpdate(tsFunction);
				List<MenuEN> subFunction1 = systemService.findByProperty(MenuEN.class, "TSFunction.id", tsFunction.getId());
	...
	@ResponseBody
	public AjaxJson saveFunction(MenuEN function, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		...
		if (function.getTSFunction().getId().equals("")) {
			...
			MenuEN parent = systemService.getEntity(MenuEN.class,
		...
		if (StringUtil.isNotEmpty(function.getId())) {
			...
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
			List<MenuEN> subFunction = systemService.findByProperty(MenuEN.class, "TSFunction.id", function.getId());
...
//			systemService.flushRoleFunciton(function.getId(), function);
			...
			if (function.getFunctionLevel().equals(Globals.Function_Leave_ONE)) {
				List<MenuEN> functionList = systemService.findByProperty(
						MenuEN.class, "functionLevel",
	...
	public ModelAndView addorupdate(MenuEN function, HttpServletRequest req) {
		String functionid = req.getParameter("id");
		List<MenuEN> fuinctionlist = systemService
				.getList(MenuEN.class);
		...
		if (functionid != null) {
			function = systemService.getEntity(MenuEN.class, functionid);
		...
		if (function.getTSFunction() != null
			...
			function.setTSFunction((MenuEN) systemService.getEntity(
					MenuEN.class, function.getTSFunction().getId()));
			req.setAttribute("function", function);
		}
		return new ModelAndView("zkyen/function/function");
	}
	...
	public List<TreeGrid> functionGrid(HttpServletRequest request,
			TreeGrid treegrid) {
		CriteriaQuery cq = new CriteriaQuery(MenuEN.class);
...
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
		...
		cq.add();
		List<MenuEN> functionList = systemService.getListByCriteriaQuery(cq, false);
	...
	public void functionList(HttpServletRequest request,
		...
		cq.add();
		List<MenuEN> functionList = systemService.getListByCriteriaQuery(
				cq, false);
		List<TreeGrid> treeGrids = new ArrayList<TreeGrid>();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
...
//			cq.eq("TSFunction.id","402881214e6b6495014e6b77aa000019");
	...
	public ModelAndView searchApp(MenuEN function, HttpServletRequest req) {
		...
		return new ModelAndView("zkyen/function/menuAppList");
	}
